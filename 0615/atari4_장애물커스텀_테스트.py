...
class GridEnvironment:
    def __init__(self, grid_size=(7, 22)):
        ...
        self.grid[4, 10] = 1 # 장애물 (4, 15)
        self.grid[1, 8] = 1 # 장애물 (4, 15)
    ...
    def step(self, action):
        ...
        if self.is_in_goal(x, y):
            ...
            if distance < self.previous_distance:
                reward += +0.1 * (self.previous_distance - distance) + front_distance * 0.1  # 목표 지점에 가까워지면 보상 증가, 진행방향 장애물 거리 비례 보상상
    ...
    def get_state(self):
        ...
        state = list(self.agent_pos) + heading_onehot + self.get_sensor_distances(self.agent_pos[0], self.agent_pos[1], self.heading)
...
model.load_state_dict(torch.load("직선으로이동_장애물도도.pth", map_location=device))
model.eval()
