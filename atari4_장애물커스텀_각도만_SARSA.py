...
def train_sarsa_with_visualization(env, max_episodes=3000):
    ...
    for episode in range(max_episodes):
        ...
        while not done:
            ...
            if len(replay_buffer) >= batch_size:
                ...
                next_actions = torch.LongTensor(next_actions).unsqueeze(1).to(device)
                ...
                with torch.no_grad():
                    next_q_values = model(next_states).gather(1, next_actions).squeeze()
    ...
    torch.save(model.state_dict(), "직선으로이동_장애물도_센서값만입력력_SARSA.pth")
